...
const { INVALID_RESETUSERNAME_PAYLOAD } = constants;

const schema = Joi.object().keys({
  username: Joi.string().email({ minDomainAtoms: 2 }),
});

async function validateResetPWDPayload(req, res, next) {
  debug(req.body)
  ...
  if (errors) {
    debug(errors)
    req.messages = { errors };
    return res.status(400).send({error:req.messages, error: INVALID_RESETUSERNAME_PAYLOAD});
  }
  return next();
}

module.exports = validateResetPWDPayload;
